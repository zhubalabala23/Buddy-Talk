...
export default function ObjectivesView({ onNext, onBack }) {
  ...
  return (
    <div className="fixed inset-0 w-full h-full overflow-y-auto flex flex-col items-center justify-start p-4 md:p-8">
      {/* Custom Background for Objectives Page */}
      <div 
        className="fixed inset-0 w-full h-full pointer-events-none bg-no-repeat bg-cover bg-center"
      ...
      <div className="relative z-10 w-full max-w-[90rem] mx-auto flex flex-col py-6 md:py-12 md:my-auto">
        ...
        >
          ...
          <p className="text-[10px] md:text-xs font-bold text-[#315588] tracking-wider uppercase">
            MENCARI SEBAB AKIBAT BERSAMA TEMAN...
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-8 xl:gap-6 mt-4 md:mt-0 px-4">
          {objectives.map((obj) => (
            <motion.div 
              ...
              className={`relative rounded-[2rem] border-[6px] md:border-8 ${obj.borderColor} bg-[#fef9eb] px-4 md:px-6 pb-8 pt-12 flex flex-col items-center text-center shadow-xl h-full ring-4 ring-white/40 ring-inset`}
            >
              {/* Leaf Number Badge */}
              ...
              <div className="w-full mt-auto relative flex flex-col justify-end items-center">
                {/* Badges for Card 3 */}
                {obj.badges && (
                  <div className="flex flex-col gap-1.5 z-30 mt-4">
